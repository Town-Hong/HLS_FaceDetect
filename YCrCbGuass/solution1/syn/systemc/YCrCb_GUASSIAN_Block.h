// ==============================================================
// RTL generated by Vivado(TM) HLS - High-Level Synthesis from C, C++ and SystemC
// Version: 2017.4
// Copyright (C) 1986-2017 Xilinx, Inc. All Rights Reserved.
// 
// ===========================================================

#ifndef _YCrCb_GUASSIAN_Block_HH_
#define _YCrCb_GUASSIAN_Block_HH_

#include "systemc.h"
#include "AESL_pkg.h"


namespace ap_rtl {

struct YCrCb_GUASSIAN_Block : public sc_module {
    // Port declarations 12
    sc_in_clk ap_clk;
    sc_in< sc_logic > ap_rst;
    sc_in< sc_logic > ap_start;
    sc_out< sc_logic > ap_done;
    sc_in< sc_logic > ap_continue;
    sc_out< sc_logic > ap_idle;
    sc_out< sc_logic > ap_ready;
    sc_in< sc_lv<64> > tmp3_dout;
    sc_in< sc_logic > tmp3_empty_n;
    sc_out< sc_logic > tmp3_read;
    sc_out< sc_lv<64> > threshold;
    sc_out< sc_logic > threshold_ap_vld;


    // Module declarations
    YCrCb_GUASSIAN_Block(sc_module_name name);
    SC_HAS_PROCESS(YCrCb_GUASSIAN_Block);

    ~YCrCb_GUASSIAN_Block();

    sc_trace_file* mVcdFile;

    sc_signal< sc_logic > ap_done_reg;
    sc_signal< sc_lv<1> > ap_CS_fsm;
    sc_signal< sc_logic > ap_CS_fsm_state1;
    sc_signal< sc_logic > tmp3_blk_n;
    sc_signal< bool > ap_block_state1;
    sc_signal< sc_lv<64> > threshold_preg;
    sc_signal< sc_lv<1> > ap_NS_fsm;
    static const sc_logic ap_const_logic_1;
    static const sc_logic ap_const_logic_0;
    static const sc_lv<1> ap_ST_fsm_state1;
    static const sc_lv<32> ap_const_lv32_0;
    static const sc_lv<64> ap_const_lv64_0;
    static const bool ap_const_boolean_1;
    // Thread declarations
    void thread_ap_clk_no_reset_();
    void thread_ap_CS_fsm_state1();
    void thread_ap_block_state1();
    void thread_ap_done();
    void thread_ap_idle();
    void thread_ap_ready();
    void thread_threshold();
    void thread_threshold_ap_vld();
    void thread_tmp3_blk_n();
    void thread_tmp3_read();
    void thread_ap_NS_fsm();
};

}

using namespace ap_rtl;

#endif
